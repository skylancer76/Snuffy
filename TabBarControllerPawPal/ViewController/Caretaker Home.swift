...
class Caretaker_Home: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    // User role IDs (set if the user is in the corresponding collection)
    ...
    func fetchCaretakerRequests() {
        ...
        FirebaseManager.shared.fetchAssignedRequests(for: caretakerId) { requests in
            ...
            DispatchQueue.main.async {
                ...
                // Present notification if there is at least one new request.
                if !requests.isEmpty {
    ...
    // MARK: - Presenting the Notification Sheet
    
    func presentNotification() {
        if let notificationVC = storyboard?.instantiateViewController(withIdentifier: "Notification_Scene") as? Notification_Scene {
    ...
    // MARK: - Profile Icon and Redirection (Unchanged)
    ...
    func setupProfileIcon(with name: String) {
        ...
        if let largeTitleView = navigationController?.navigationBar.subviews.first(where: { String(describing: type(of: $0)) == "_UINavigationBarLargeTitleView" }) {
    ...
    @objc func profileTapped() {
        if let profileVC = storyboard?.instantiateViewController(withIdentifier: "Profile") as? User_Profile {
    ...
    func redirectToLogin() {
        do {
            ...
            if let loginVC = storyboard?.instantiateViewController(withIdentifier: "login") as? User_Login {

...
class My_Bookings: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        // Observe caretaker bookings (real-time).
        bookingsListener = FirebaseManager.shared.observeOwnerBookings(for: ownerId) { [weak self] requests in
            guard let self = self else { return }
            self.caretakerUpcomingBookings = requests.filter { $0.status != "Completed" }
            self.caretakerCompletedBookings = requests.filter { $0.status == "Completed" }
        ...
        // Observe dog walker bookings (real-time).
        ...
        // Set up a gradient background (unchanged).
        let gradientView = UIView(frame: view.bounds)
        gradientView.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(gradientView)
        view.sendSubviewToBack(gradientView)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if bookingSegmentedControl.selectedSegmentIndex == 0 {
            ...
            // Existing caretaker cell configuration
            cell.configureCell(with: request)
            
            // Make the status button smaller using configuration
            cell.statusButton.layer.cornerRadius = 6
            cell.statusButton.clipsToBounds = true
            cell.statusButton.titleLabel?.font = UIFont.systemFont(ofSize: 9)
            if var config = cell.statusButton.configuration {
                config.contentInsets = NSDirectionalEdgeInsets(top: 4, leading: 8, bottom: 4, trailing: 8)
                cell.statusButton.configuration = config
            }
            
            // Format the status text: "Pending", "Accepted", "Ongoing", "Completed"
            ...
            // Existing dog walker cell configuration
            cell.configureCell(with: request)
            
            // Make the status button smaller
            cell.statusButton.layer.cornerRadius = 6
            cell.statusButton.clipsToBounds = true
            cell.statusButton.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            cell.statusButton.contentEdgeInsets = UIEdgeInsets(top: 4, left: 8, bottom: 4, right: 8)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // Check for pending status: if pending, do not navigate.
        if bookingSegmentedControl.selectedSegmentIndex == 0 {
            let request = allCaretakerBookings[indexPath.row]
